...
function FinancialMonthModal({
  ...
  const forecastPercent = numberInput(form.forecastPercent);
  const actualPercent = numberInput(form.actualPercent);
  const forecastAmount = Math.round((projectContractValue * forecastPercent) / 100);
  const actualAmount = Math.round((projectContractValue * actualPercent) / 100);
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-white/[0.07] px-5 py-4">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-600">Nhập Forecast %, Actual % và Revenue theo tháng. Tổng KPI không cộng dồn phần trăm giữa các tháng.</div>
          </div>

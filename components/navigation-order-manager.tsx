...
export function NavigationOrderManager({
  ...
  return (
    <div className="asc-modal-viewport fixed inset-0 z-[230] flex items-center justify-center bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true" data-modal-lock="true">
      <div className="asc-modal-panel tech-panel flex w-full max-w-[760px] flex-col rounded-2xl shadow-2xl">
        <div className="asc-modal-header flex items-center justify-between border-b border-white/[0.06] px-5 py-4"><div><div className="text-sm font-semibold text-white">Project Command Center • Navbar Modules</div><div className="mt-1 text-[10px] text-slate-600">Kéo thả để đổi vị trí. Tên gốc do hệ thống quản lý, tên hiển thị có thể đổi theo cách làm việc của bạn.</div></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-xl border border-white/[0.07] text-slate-500"><X className="size-4" /></button></div>
        <div className="asc-modal-scroll scrollbar-thin space-y-2 p-4">

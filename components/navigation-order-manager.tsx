...
export function NavigationOrderManager({
  ...
  return (
    <div className="fixed inset-0 z-[230] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="tech-panel w-full max-w-[560px] overflow-hidden rounded-2xl shadow-2xl">
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4"><div><div className="text-sm font-semibold text-white">Sắp xếp menu trái</div><div className="mt-1 text-[10px] text-slate-600">Kéo thả hoặc dùng mũi tên để thay đổi vị trí module.</div></div><button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-xl border border-white/[0.07] text-slate-500"><X className="size-4" /></button></div>
        <div className="scrollbar-thin max-h-[65vh] space-y-2 overflow-y-auto p-4">
          {value.map((href, index) => <div key={href} draggable onDragStart={() => setDragged(href)} onDragEnd={() => { setDragged(null); setOver(null); }} onDragOver={(event) => { event.preventDefault(); setOver(href); }} onDrop={(event) => { event.preventDefault(); drop(href); }} className={cn("grid grid-cols-[28px_30px_1fr_auto] items-center gap-2 rounded-xl border px-3 py-2.5 transition", over === href && dragged !== href ? "border-cyan-300/30 bg-cyan-300/[0.06]" : "border-white/[0.06] bg-white/[0.018]")}><GripVertical className="size-4 cursor-grab text-slate-600 active:cursor-grabbing" /><span className="grid size-6 place-items-center rounded-lg bg-white/[0.035] text-[9px] text-slate-600">{index + 1}</span><div><div className="text-xs font-medium text-slate-300">{labels[href] ?? href}</div><div className="mt-0.5 font-mono text-[8px] text-slate-700">{href}</div></div><div className="flex gap-1"><button type="button" disabled={index === 0} onClick={() => move(href, -1)} className="grid size-8 place-items-center rounded-lg border border-white/[0.06] text-slate-600 disabled:opacity-20"><ChevronUp className="size-3.5" /></button><button type="button" disabled={index === value.length - 1} onClick={() => move(href, 1)} className="grid size-8 place-items-center rounded-lg border border-white/[0.06] text-slate-600 disabled:opacity-20"><ChevronDown className="size-3.5" /></button></div></div>)}
        </div>
        <div className="flex items-center border-t border-white/[0.06] px-5 py-4"><button type="button" onClick={() => onChange([...DEFAULT_NAV_ORDER])} className="flex h-9 items-center gap-2 rounded-xl border border-white/[0.07] px-3 text-[10px] text-slate-500"><RotateCcw className="size-3.5" /> Mặc định</button><button type="button" disabled={saving} onClick={onSave} className="ml-auto flex h-9 items-center gap-2 rounded-xl bg-cyan-300 px-4 text-[10px] font-semibold text-[#07111f] disabled:opacity-50"><Save className="size-3.5" /> {saving ? "Đang lưu..." : "Lưu vị trí"}</button></div>
      </div>
    </div>
  );
}

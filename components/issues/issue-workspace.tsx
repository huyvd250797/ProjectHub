...
export function IssueWorkspace() {
  ...
  async function duplicateSelected() {
    ...
    try {
      const response = await fetch("/api/issues", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        projectId: selectedProject.id, content: issue.content, statusCode: "waiting", customerStatusCode: "not_handed_over", priorityCode: issue.priorityCode ?? "B", stageCode: issue.stageCode, dueDate: issue.dueDate, estimatedHours: issue.estimatedHours, moduleId: issue.moduleId, departmentId: issue.departmentId, requesterId: issue.requesterId, assigneeId: issue.assigneeId, notes: issue.notes ? `${issue.notes}\nNhân bản từ ISSUE #${issue.issueNo ?? "—"}` : `Nhân bản từ ISSUE #${issue.issueNo ?? "—"}`,
      }) });
      const body = (await response.json()) as IssueMutationResponse;
      if (!body.ok) throw new Error(body.message);
  ...
  function renderCell(issue: IssueRow, id: IssueColumnId) {
    ...
    if (id === "stage") return <FloatingSelect ariaLabel="Giai đoạn" compact disabled={editingDisabled} value={issue.stageCode} options={currentData.lookups.stages} onChange={(value) => inlineUpdate(issue, "stageCode", value)} placeholder="Chưa gán" />;
    ...
    if (id === "requester") return <FloatingSelect ariaLabel="Nhân sự yêu cầu" compact disabled={editingDisabled} value={issue.requesterId} options={currentData.lookups.requesters} onChange={(value) => inlineUpdate(issue, "requesterId", value)} placeholder="Chưa xác định" />;
    if (id === "assignee") {
    ...
    if (id === "releaseDate") return <span className="text-[10px] text-slate-600">{formatDate(issue.releaseDate)}</span>;
    if (id === "estimatedHours") return <span className="font-mono text-[10px] text-slate-400">{issue.estimatedHours === null ? "—" : `${issue.estimatedHours.toLocaleString("vi-VN")}h`}</span>;
    if (id === "response") return issue.response ? <span className="whitespace-normal break-words text-[11px] leading-5 text-slate-400">{issue.response}</span> : <span className="text-slate-800">—</span>;
    if (id === "notes") return issue.notes ? <span className="whitespace-normal break-words text-[11px] leading-5 text-slate-500">{issue.notes}</span> : <span className="text-slate-800">—</span>;
            ...
            <tbody>
              {data.rows.length ? data.rows.map((issue) => (
                <tr key={issue.id} onClick={() => openIssue(issue)} className={cn("group cursor-pointer border-b border-white/[0.04] text-xs text-slate-400 transition hover:bg-white/[0.025]", selectedIds.has(issue.id) && "bg-cyan-300/[0.025]") }>
                  ...
                  {orderedVisibleColumns.map((id) => { const width = preferences.columnWidths[id] ?? 160; const left = pinnedLeft(id); const pinned = left !== undefined; const interactive = ["status","customerStatus","priority","stage","module","department","requester","assignee","jira"].includes(id); return <td key={id} className={cn("px-3 py-3.5 align-top", pinned && "sticky z-10 border-r border-white/[0.045] bg-[#0b1727]")} style={{ width, minWidth: width, maxWidth: width, left }} onClick={interactive ? (e) => e.stopPropagation() : undefined}><div className="max-w-full whitespace-normal break-words">{renderCell(issue, id)}</div></td>; })}
                </tr>

...
function Modal({ title, subtitle, onClose, children }: { title: string; subtitle: string; onClose?: () => void; children: ReactNode }) {
  return <div className="asc-modal-viewport fixed inset-0 z-[80] flex items-center justify-center bg-black/70 backdrop-blur-sm" role="dialog" aria-modal="true" data-modal-lock="true"><div className="asc-modal-panel flex w-full max-w-[880px] flex-col rounded-2xl border border-white/[0.1] bg-[#0a1828] shadow-2xl"><div className="asc-modal-header flex items-start justify-between border-b border-white/[0.07] bg-[#0a1828]/95 px-5 py-4 backdrop-blur"><div><div className="text-base font-semibold text-white">{title}</div><div className="mt-1 text-xs text-slate-500">{subtitle}</div></div>{onClose ? <button type="button" onClick={onClose} className="grid size-9 place-items-center rounded-xl border border-white/[0.08] text-slate-400 hover:text-white" aria-label="Đóng modal"><X className="size-4" /></button> : null}</div><div className="asc-modal-scroll scrollbar-thin p-5">{children}</div></div></div>;
}

    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-cyan-300/12 bg-cyan-300/[0.035] p-4 text-xs leading-5 text-slate-400">
        <div className="flex items-start gap-3">
          ...
          <div>
            Project đích đang chọn: <span className="font-semibold text-amber-200">{selectedProject.code}</span> • {selectedProject.organizationName || selectedProject.name}. Dry-run chỉ kiểm tra dữ liệu; chưa ghi database.
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-2 max-w-xl text-xs leading-6 text-slate-500">
            Upload workbook dự án theo template hiện tại. Hệ thống phân tích sheet, record count, mapping, dữ liệu thiếu và xung đột project trước khi cho phép Apply Import ở phiên bản sau.
          </p>
          <button
            type="button"
          ...
          <div className="mt-3 text-[10px] text-slate-700">Tối đa 10 MB • endpoint không lưu file upload</div>
        </div>
      </div>
        ...
        <>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="mt-1 text-[11px] text-slate-500">
                  Workbook nhận diện: <span className="font-semibold text-amber-200/80">{result.sourceProject.code || "—"}</span> • Project đích: <span className="font-semibold text-cyan-200/80">{result.selectedProject?.code || selectedProject.code}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <section className="grid grid-cols-1 gap-4 xl:grid-cols-[1.08fr_0.92fr]">
            ...
            <div className="tech-panel rounded-2xl p-4">
              ...
              <div className="mt-4 flex items-start gap-2 rounded-xl border border-amber-300/12 bg-amber-300/[0.04] p-3">
                <LockKeyhole className="mt-0.5 size-3.5 shrink-0 text-amber-200/70" />
                <p className="text-[10px] leading-5 text-slate-500">
                  Remote password/token/secret bị loại khỏi payload: {result.quality.sensitiveColumnsExcluded.join(", ")}.
                </p>
              </div>
            </div>
          </section>
          ...
          {result.messages.length ? (
            <section className="tech-panel overflow-hidden rounded-2xl">
              <div className="border-b border-white/[0.06] px-4 py-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">Dry-run messages</div>
              {result.messages.map((message, index) => (
                <div key={`${message.code}-${index}`} className="flex items-start gap-3 border-b border-white/[0.04] px-4 py-3 text-xs">
                  {message.severity === "error" ? <XCircle className="mt-0.5 size-3.5 shrink-0 text-rose-300" /> : message.severity === "warning" ? <AlertTriangle className="mt-0.5 size-3.5 shrink-0 text-amber-300" /> : <Info className="mt-0.5 size-3.5 shrink-0 text-cyan-300/70" />}
                  <div>
                    <div className="font-medium text-slate-400">{message.message}</div>
                    <div className="mt-1 font-mono text-[9px] uppercase tracking-[0.12em] text-slate-700">{message.code}{message.sheet ? ` • ${message.sheet}` : ""}{message.row ? ` • row ${message.row}` : ""}</div>
                  </div>
                </div>
              ))}
            </section>

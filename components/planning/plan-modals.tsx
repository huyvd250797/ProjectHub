  ...
  return (
    <ModalShell eyebrow="Master Plan" title={plan ? "Cập nhật Master Plan" : "Khởi tạo Master Plan"} description="Ngày bắt đầu và cách tính ngày sẽ điều khiển lịch tuần tự của toàn bộ Project Stages." icon={<Target className="size-5" />} saving={saving} onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div><FieldLabel required>Ngày bắt đầu</FieldLabel><DateInput className="field" value={startDate} onChange={setStartDate} /><FieldError message={fieldErrors.startDate} /></div>
        <div><FieldLabel>Ngày kết thúc mục tiêu</FieldLabel><DateInput className="field" min={startDate || undefined} value={targetEndDate} onChange={setTargetEndDate} /><FieldError message={fieldErrors.targetEndDate} /></div>
  ...
  return (
    <ModalShell eyebrow="Auto Generate Plan" title="Tự sinh stage, số ngày và milestone" description="Nhập ngày bắt đầu/kết thúc để app đề xuất timeline triển khai. Sau khi tạo, bạn vẫn chỉnh thủ công từng stage và milestone như bình thường." icon={<Sparkles className="size-5" />} saving={saving} submitLabel="Tạo kế hoạch" onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div><FieldLabel required>Ngày bắt đầu</FieldLabel><DateInput className="field" value={startDate} onChange={(value) => { setStartDate(value); if (targetEndDate && value && targetEndDate < value) setTargetEndDate(value); }} /><FieldError message={fieldErrors.startDate} /></div>
        <div><FieldLabel required>Ngày kết thúc</FieldLabel><DateInput className="field" min={startDate || undefined} value={targetEndDate} onChange={setTargetEndDate} /><FieldError message={fieldErrors.targetEndDate} /></div>
    ...
    <ModalShell eyebrow="Project Stages" title={stage ? `Cập nhật ${stage.code}` : "Thêm Project Stage"} description="Nhập trực tiếp Từ ngày – Đến ngày, hoặc để hệ thống xếp lịch tự động theo Master Plan." icon={<Layers3 className="size-5" />} saving={saving} onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div className="md:col-span-2 grid grid-cols-1 gap-5 md:grid-cols-3">
          <div><FieldLabel required={dateMode === "manual"}>Từ ngày</FieldLabel><DateInput required={dateMode === "manual"} disabled={dateMode === "auto"} className="field disabled:cursor-not-allowed disabled:opacity-55" value={startDate} onChange={(value) => { setStartDate(value); if (endDate && value && endDate < value) setEndDate(value); }} /><FieldError message={fieldErrors.startDate} /></div>
          <div><FieldLabel required={dateMode === "manual"}>Đến ngày</FieldLabel><DateInput required={dateMode === "manual"} min={startDate || undefined} disabled={dateMode === "auto"} className="field disabled:cursor-not-allowed disabled:opacity-55" value={endDate} onChange={setEndDate} /><FieldError message={fieldErrors.endDate} /></div>
  ...
  return (
    <ModalShell eyebrow="Milestone" title={milestone ? "Cập nhật Milestone" : "Thêm Milestone"} description="Đánh dấu một mốc phê duyệt, bàn giao hoặc quyết định quan trọng trên timeline." icon={<Flag className="size-5" />} saving={saving} onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div><FieldLabel required>Ngày milestone</FieldLabel><DateInput className="field" value={dueDate} onChange={setDueDate} /><FieldError message={fieldErrors.dueDate} /></div>
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await readMutation(await fetch(task ? `/api/plan/tasks/${task.id}` : "/api/plan/tasks", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ projectId, title, description, stageId, status, priority, dueDate, estimatedHours: estimatedHours === "" ? null : Number(estimatedHours), ownerId, sortOrder: task?.sortOrder ?? null }),
      }));
      onSaved(result.message);
    } catch (reason) {
      const parsed = mutationError(reason); setError(parsed.message); setFieldErrors(parsed.fieldErrors);
  ...
  return (
    <ModalShell eyebrow="Execution Task" title={task ? "Cập nhật task" : "Thêm task thực thi"} description="Chia nhỏ stage thành đầu việc có deadline, mức ưu tiên và trạng thái để theo dõi khi triển khai." icon={<ClipboardList className="size-5" />} saving={saving} onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div><FieldLabel>Deadline</FieldLabel><DateInput className="field" value={dueDate} onChange={setDueDate} /><FieldError message={fieldErrors.dueDate} /></div>
        <div><FieldLabel>Giờ ước tính</FieldLabel><input type="number" min="0" max="9999" step="0.25" className="field" value={estimatedHours} onChange={(event) => setEstimatedHours(event.target.value)} placeholder="Ví dụ: 6" /><FieldError message={fieldErrors.estimatedHours} /></div>
  ...
  return (
    <ModalShell eyebrow="Smart Reminders" title={reminder ? "Cập nhật reminder" : "Thêm reminder"} description="Tạo nhắc việc cho Master Plan, stage, milestone, task hoặc ISSUE liên quan đến kế hoạch." icon={<BellRing className="size-5" />} saving={saving} onClose={onClose} onSubmit={submit}>
      ...
      <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
        ...
        <div><FieldLabel required>Thời điểm nhắc</FieldLabel><DateTimeInput className="field" value={remindAt} onChange={setRemindAt} /><FieldError message={fieldErrors.remindAt} /></div>
        ...
        <div><FieldLabel required={status === "snoozed"}>Snooze đến</FieldLabel><DateTimeInput disabled={status !== "snoozed"} className="field disabled:cursor-not-allowed disabled:opacity-55" value={snoozedUntil} onChange={setSnoozedUntil} /><FieldError message={fieldErrors.snoozedUntil} /></div>

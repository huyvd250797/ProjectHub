...
import { normalizeNavigationOrder } from "@/lib/workspace-preferences";
...
export async function GET() {
  ...
  if (!supabase) {
    return NextResponse.json({ ok: true, source: "demo", preferences: { navigationOrder: normalizeNavigationOrder(null) } } satisfies WorkspacePreferencesApiResponse);
  }
  ...
  if (error) {
    if (/workspace_user_preferences|does not exist|schema cache/i.test(error.message)) {
      return NextResponse.json({ ok: true, source: "database", preferences: { navigationOrder: normalizeNavigationOrder(null) } } satisfies WorkspacePreferencesApiResponse);
    }
  ...
  return NextResponse.json({ ok: true, source: "database", preferences: { navigationOrder: normalizeNavigationOrder(data?.navigation_order) } } satisfies WorkspacePreferencesApiResponse);
}
...
export async function PUT(request: NextRequest) {
  ...
  const preferences = { navigationOrder: normalizeNavigationOrder(body.navigationOrder) };

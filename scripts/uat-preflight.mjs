...
for (const token of ["PLHD_COLUMN_STORAGE_KEY", "columnOrder", "columnWidths", "draggable", "startResize", "whitespace-normal"]) {
  contractViewV220.includes(token) ? pass(`V2.2.1 PLHĐ grid UX: ${token}`) : fail(`V2.2.1 PLHĐ grid UX: ${token}`);
}
...
for (const token of ["GlobalGridEnhancer", "MutationObserver", "moveColumn", "setColumnWidth", "localStorage", "asc-data-grid"]) {
  globalGridEnhancerV222.includes(token) ? pass(`V2.4.1 Global grid enhancer: ${token}`) : fail(`V2.4.1 Global grid enhancer: ${token}`);
}
...
for (const token of ["resizeColumn", "data-managed-grid=\"true\"", "whitespace-normal break-words", "cursor-col-resize"]) {
  issueWorkspace.includes(token) ? pass(`V2.4.1 ISSUE column resize/wrap: ${token}`) : fail(`V2.4.1 ISSUE column resize/wrap: ${token}`);
}
...
for (const token of ["Workload", "/workload", "UsersRound"]) {
  navigationV240.includes(token) ? pass(`V2.4.1 Workload navigation: ${token}`) : fail(`V2.4.1 Workload navigation: ${token}`);
}

const preferencesV240 = fs.readFileSync(path.join(root, "lib/workspace-preferences.ts"), "utf8");
preferencesV240.includes("\"/workload\"") ? pass("V2.4.1 Workload navbar preference") : fail("V2.4.1 Workload navbar preference");
...
for (const token of ["WorkloadMember", "capacityScore", "WorkloadAssignmentSuggestion", "WorkloadCalendarBucket"]) {
  workloadTypesV240.includes(token) ? pass(`V2.4.1 Workload types: ${token}`) : fail(`V2.4.1 Workload types: ${token}`);
}
...
for (const token of ["loadWorkloadData", "project_plan_tasks", "project_plan_reminders", "assignee_person_id", "overloaded", "buildCalendarBuckets"]) {
  workloadServerV240.includes(token) ? pass(`V2.4.1 Workload server: ${token}`) : fail(`V2.4.1 Workload server: ${token}`);
}
...
for (const token of ["createDemoWorkload", "loadWorkloadData", "getEffectiveProjectRole", "WORKLOAD_QUERY_FAILED"]) {
  workloadApiV240.includes(token) ? pass(`V2.4.1 Workload API: ${token}`) : fail(`V2.4.1 Workload API: ${token}`);
}
...
const exportHelperV370 = fs.readFileSync(path.join(root, "lib/export-data.ts"), "utf8");
for (const token of ["exportCsv", "exportCsvSections", "\\uFEFF", "safeFilename"]) {
  exportHelperV370.includes(token) ? pass(`V3.7.0 export helper: ${token}`) : fail(`V3.7.0 export helper: ${token}`);
}

const exportModulesV370 = [
  "components/dashboard/project-dashboard.tsx",
  "components/finance/project-financial-control.tsx",
  "components/contract-view.tsx",
  "components/departments/department-intelligence.tsx",
  "components/workload/workload-dashboard.tsx",
  "components/resource-scheduling/assignment-board.tsx",
  "components/documents/project-documents.tsx",
  "components/resources/resource-vault.tsx",
  "components/command-center/project-command-center.tsx",
  "components/portfolio/portfolio-dashboard.tsx",
  "components/master/master-project-console.tsx",
  "components/notifications/activity-center.tsx",
...
for (const token of ["Download", "Export", "exportCsv", "exportCsvSections", "resource-safe-metadata-export"]) {
  const source = token === "resource-safe-metadata-export" ? fs.readFileSync(path.join(root, "app/api/health/route.ts"), "utf8") : exportModulesV370;
  source.includes(token) ? pass(`V3.7.0 module export: ${token}`) : fail(`V3.7.0 module export: ${token}`);
}

const docsV370 = [
  fs.readFileSync(path.join(root, "docs/V3.7.0-SCOPE.md"), "utf8"),
  fs.readFileSync(path.join(root, "docs/UAT_V370_MODULE_DATA_EXPORT_CHECKLIST.md"), "utf8"),
].join("\n");
for (const token of ["Module Data Export", "Dashboard", "Finance", "PLHĐ", "Resource Vault", "không export secret/password"]) {
